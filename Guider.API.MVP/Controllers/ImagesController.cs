...
namespace Guider.API.MVP.Controllers
{
    ...
    public class ImagesController : ControllerBase
    {
        ...
        public async Task<IActionResult> UploadImage([FromForm] ImageUploadRequest request)
        {
            ...
            Console.WriteLine($"Place: {request?.Place}");
            Console.WriteLine($"ImageName: {request?.ImageName}");
            Console.WriteLine($"ImageFile: {request?.ImageFile?.FileName} ({request?.ImageFile?.Length} bytes)");
            ...
            if (request == null)
            {
                ...
                {
                    error = "Пустой запрос",
                    message = "Объект запроса не может быть null"
                });
            }
            ...
            if (string.IsNullOrEmpty(request.Province))
            {
                return BadRequest(new
                {
                    error = "Отсутствует Province",
                    message = "Province является обязательным полем"
                });
            }
            ...
            if (string.IsNullOrEmpty(request.Place))
            {
                return BadRequest(new
                {
                    error = "Отсутствует Place",
                    message = "Place является обязательным полем"
                });
            }
            ...
            if (string.IsNullOrEmpty(request.ImageName))
            {
                return BadRequest(new
                {
                    error = "Отсутствует ImageName",
                    message = "ImageName является обязательным полем"
                });
            }
            ...
            if (request.ImageFile == null)
            {
                return BadRequest(new
                {
                    ...
                    message = "ImageFile является обязательным полем"
                });
            }
            ...
            if (!allowedTypes.Contains(request.ImageFile.ContentType?.ToLower()))
            {
                return BadRequest(new
                {
                    ...
                    message = $"Тип файла {request.ImageFile.ContentType} не поддерживается. Разрешены: {string.Join(", ", allowedTypes)}"
                });
            }
            ...
            catch (Exception ex)
            {
                ...
                return StatusCode(500, new
                {
                    error = "Внутренняя ошибка сервера",
                    message = "Произошла непредвиденная ошибка при загрузке изображения",
                    details = ex.Message // Для отладки, уберите в продакшене
                });
            }
        }
