...
namespace Guider.API.MVP.Services
{
    public class ImageService : IImageService
    {
        ...
        //public async Task<JsonDocument> UpdateImageAsync(string id, string newImageName,
        ...
        //        {
        //            // Проверка на дубликаты имен файлов (исключая текущий)
        //            string imageNameWithoutExtension = Path.GetFileNameWithoutExtension(newImageName);
        //            var duplicateFilter = Builders<BsonDocument>.Filter.And(
        //                Builders<BsonDocument>.Filter.Regex("ImageName",
        //                    new BsonRegularExpression($"^{Regex.Escape(imageNameWithoutExtension)}", "i")),
        ...
        //            updates.Add(updateBuilder.Set("ImageName", newImageName));
        //        }

        //        // Обновляем файл, если предоставлен новый
        //        if (newImageFile != null && newImageFile.Length > 0)
        //        {
        //            // Проверка расширения файла
        //            string[] allowedExtensions = { ".jpg", ".jpeg", ".png", ".gif", ".bmp", ".webp" };
        //            string fileExtension = Path.GetExtension(newImageFile.FileName).ToLowerInvariant();
        ...
        //            // Удаляем старый файл
        //            string oldFilePath = existingRecord["FilePath"].AsString;
        //            string oldAbsolutePath = Path.Combine(_baseImagePath, oldFilePath);
        ...
        //            string newProvince = province ?? existingRecord.GetValue("Province", "").AsString;
        //            string newCity = city ?? (existingRecord.Contains("City") && !existingRecord["City"].IsBsonNull ? existingRecord["City"].AsString : null);
        //            string newPlace = place ?? existingRecord.GetValue("Place", "").AsString;

        //            string directoryPath = BuildDirectoryPath(newProvince, newCity, newPlace);
        //            if (!Directory.Exists(directoryPath))
        //            {
        //                Directory.CreateDirectory(directoryPath);
        //            }

        //            string fileName = !string.IsNullOrEmpty(newImageName) ? newImageName : existingRecord["ImageName"].AsString;
        //            string fullFileName = string.IsNullOrEmpty(Path.GetExtension(fileName))
        //                ? $"{fileName}{fileExtension}"
        //                : fileName;

        //            string newFullPath = Path.Combine(directoryPath, fullFileName);
        //            string newRelativePath = BuildRelativePath(newProvince, newCity, newPlace, fullFileName);
        ...
        //            updates.Add(updateBuilder.Set("FilePath", newRelativePath));
        ...
        //            updates.Add(updateBuilder.Set("Extension", fileExtension));
        //        }
        ...
        //            await _imageCollection.UpdateOneAsync(filter, combinedUpdate);
        //        }
