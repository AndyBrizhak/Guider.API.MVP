    ...
    public class ImageService : IImageService
    {
            ...
            string? newImageName = null, IFormFile? newImageFile = null)
        {
            try
            {
                ...
                // Ищем существующую запись в MongoDB
                var filter = Builders<BsonDocument>.Filter.And(
                    Builders<BsonDocument>.Filter.Eq("Province", oldProvince),
                    Builders<BsonDocument>.Filter.Eq("Place", oldPlace),
                    Builders<BsonDocument>.Filter.Eq("ImageName", oldImageName),
                    Builders<BsonDocument>.Filter.Eq("IsActive", true)
                );
                ...
                {
                    filter = Builders<BsonDocument>.Filter.And(filter,
                        Builders<BsonDocument>.Filter.Eq("City", oldCity));
                }

                var existingRecord = await _imageCollection.Find(filter).FirstOrDefaultAsync();
                if (existingRecord == null)
                {
                    return CreateJsonResponse(false, "Исходное изображение не найдено в базе данных");
                }

                // Используем новые значения, если они указаны, иначе старые
                string provinceToUse = newProvince ?? oldProvince;
                string? cityToUse = newCity ?? oldCity;
                string placeToUse = newPlace ?? oldPlace;
                string imageNameToUse = newImageName ?? oldImageName;
                ...
                if (newImageFile != null)
                {
                    ...
                    {
                        return CreateJsonResponse(false, $"Недопустимое расширение файла: {extension}");
                    }
                }
                else
                {
                ...
                string newDirectoryPath = BuildDirectoryPath(provinceToUse, cityToUse, placeToUse);
                string newRelativePath = BuildRelativePath(provinceToUse, cityToUse, placeToUse, fullImageName);
                string newAbsolutePath = Path.Combine(_baseImagePath, newRelativePath);
                ...
                if (newImageFile != null)
                {
                    ...
                    // Если путь изменился, удаляем старый файл
                    if (newAbsolutePath != oldAbsolutePath && File.Exists(oldAbsolutePath))
                    {
                        File.Delete(oldAbsolutePath);
                    }
                }
                else if (newAbsolutePath != oldAbsolutePath)
                {
                    // Перемещаем файл
                    if (File.Exists(oldAbsolutePath))
                    {
                ...
                {
                    updateBuilder.Set("Province", provinceToUse),
                    updateBuilder.Set("City", cityToUse ?? (BsonValue)BsonNull.Value),
                    updateBuilder.Set("Place", placeToUse),
                    updateBuilder.Set("ImageName", fullImageName),
                    updateBuilder.Set("FilePath", newRelativePath),
                    updateBuilder.Set("Extension", extension),
                    updateBuilder.Set("UpdatedDate", DateTime.UtcNow)
                };
                ...
                if (newImageFile != null)
                {
                    ...
                    {
                        updateBuilder.Set("OriginalFileName", newImageFile.FileName),
                        updateBuilder.Set("FileSize", newImageFile.Length),
                        updateBuilder.Set("ContentType", newImageFile.ContentType)
                    });
                }
